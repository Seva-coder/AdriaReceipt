...
class EditShopDialog : DialogFragment() {
    ...
    companion object {
        const val TAG = "EditShopDialog"
        private const val KEY_ORIG_NAME = "name"
        private const val KEY_ID = "id"

        fun newInstance(id: Long, origName: String): EditShopDialog {
            val args = Bundle()
            args.putLong(KEY_ID, id)
            args.putString(KEY_ORIG_NAME, origName)
            val fragment = EditShopDialog()
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        return activity?.let {
            ...
            val origName = requireArguments().getString(KEY_ORIG_NAME)

            val dialogView: View = binding.root

            builder.setView(dialogView)
            builder.setTitle(getString(R.string.rename_shop_dialog_title, origName))
                .setPositiveButton(getString(R.string.rename_dialog_btn)) { _, _ ->
                    val name = binding.shopName.text.toString()
                    if (name != "") {
                        vm.setShopName(id = idDB, newName = name)
                    } else {
